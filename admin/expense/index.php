...
WHERE c.status = 1 
GROUP BY c.id, c.category, c.balance, YEAR(r.date_created), expense_title, total_sub_spent.total
ORDER BY c.category, YEAR(r.date_created) DESC, expense_title;
");
                    ...
                    while ($row = $qry->fetch_assoc()):
                        ...
                        if ($category_has_expense && $current_category != $row['category']):
                            ...
                            echo '<div class="col-5 ml-2"><strong>' . $current_category .'    ' .  $row['year'] . '</strong></div>';
